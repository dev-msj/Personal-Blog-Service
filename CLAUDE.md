...
- docs/context/{overview,domain,constraints,unknowns}.md — 비즈니스 맥락, 기술 제약, Ubiquitous Language, 알려진 불확실성
- docs/problem/{overview,use-cases,domain-spec,threat-model}.md — BP1~BP6, TP1~TP8, UC, Invariant, Phase 근거, STRIDE
- docs/solution/common/overview.md — 단일 BC, 기술 스택 확정, Extension 적용 계획, Phase 정의
- docs/solution/common/application-arch.md — 최종 형상 모듈 구조, Aggregate, 채택 패턴, 3방향 리팩토링
- docs/solution/common/data-design.md — 최종 형상 스키마/DDL/ER, 트랜잭션, Redis 키 구조
- docs/solution/common/async.md — 이벤트 계약, 브로커(BullMQ+Kafka), Outbox, Idempotency(이벤트), DLQ
- docs/solution/common/security.md — 인증/인가, 시크릿, 데이터 보호, Rate Limiting, API Idempotency-Key
- docs/solution/common/observability.md — 로그/메트릭/트레이싱, Correlation ID, 감사 로그(audit_log), LGTM 스택
- docs/solution/phase-{N}/scope.md, arch-increment.md, data-migration.md, <topic>-deployment.md — N번째 Phase의 증분 절차
- docs/implementation/issue-plan.md — 현재 Phase의 이슈 분해와 의존 관계
- docs/qa/e2e-catalog.md — UC별 E2E 검증의 GWT 서술형 living documentation (수동 QA 대체 게시 채널). 테스트 코드가 primary, 카탈로그는 [E2E 검증] 이슈 B4에서 재생성되는 복제본 (직접 수정 금지)
