...
- #128 [리팩토링, 보안] U1: AuthGuard sub BIGINT 변환 + JwtService.verifyRefreshToken throw 통일 (#70 본체 흡수)
  ...
  migration: parallel-change/migrate
  note: @AuthenticatedUserValidation 식별자(uid email→user_id)를 소비하는 컨트롤러 3곳(post #121, post-like #122, user-info #155)과 결합된 migrate 클러스터(순서는 각 이슈 consumes ←#128로 표기). 그린 게이트 결정: 데코레이터 반환을 number로 바꾸면 3 컨트롤러가 #128 머지 시 동시 컴파일 깨짐(원자 웨이브 필요) / 반환 string 유지(헤더 원형) + 소비처 parseInt면 타입 호환 유지(권장, 소비처는 #121·#122·#155가 user_id 의미로 전환). 어느 쪽이든 세 이슈가 #128을 선행 소비. work B3에서 데코레이터 전략 확정 — testing-strategy.md §13
